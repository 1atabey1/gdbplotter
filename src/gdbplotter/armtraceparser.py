...
class TraceParser(GdbParser):
    ...
    - ARM CoreSight Components Technical Reference Manual (ARM DDI 0314H)
      https://developer.arm.com/documentation/ddi0314/h/

    - ARM CoreSight ETB-R4 Technical Reference Manual (ARM DDI 0541A)
      https://developer.arm.com/documentation/ddi0541/a

    - ARMv7-M Architecture Reference Manual (ARM DDI 0403E)
      https://developer.arm.com/documentation/ddi0403/e/

    - Cortex-M3/M4/M7 Technical Reference Manuals:
      * Cortex-M3: ARM DDI 0337E
      * Cortex-M4: ARM DDI 0439B
      * Cortex-M7: ARM DDI 0489D

    - ARM Embedded Trace Macrocell Architecture Specification (ARM IHI 0014Q)
      https://developer.arm.com/documentation/ihi0014/q

    Standard CoreSight Memory Map (ARMv7-M):
    ...
    NUM_DWT_COMPARATORS = 4  # Most Cortex cores have 4 but not sure if all do
    ...
    def _read_etb_buffer(self) -> bytes:
        ...
        try:
            ...
            if rwp > rrp:
                ...
                trace_stream = bytearray(self._read_memory(self.ETB_RAM + start_offset, num_words * 4))
            else:
                ...
                if words_to_end > 0:
                    chunk1 = self._read_memory(self.ETB_RAM + (rrp * 4), words_to_end * 4)
                    trace_stream.extend(chunk1)

                # Read second chunk (start to rwp)
                if rwp > 0:
                    chunk2 = self._read_memory(self.ETB_RAM, rwp * 4)
    ...
    def _parse_trace_packets(self, trace_stream: bytes) -> dict:
        ...
        while offset < len(trace_stream):
            ...
            if (header & 0x04) == 0x04:  # DWT event/data trace indicator
                comp_num, data_bytes, consumed = self._parse_dwt_packet(header, b"", offset, trace_stream)
    ...
    def start(self):
        ...
        if not self._configure_trace_infrastructure():
            ...
            for idx, region in enumerate(self.regions):
                if idx >= self.NUM_DWT_COMPARATORS:  # Most Cortex-M have 4 comparators
